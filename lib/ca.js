...
var CA = function (caFolder) {
  ...
  this.rootCAFolder = this.baseCAFolder + "/rootca";
  ...
  try {
    FS.mkdirSync(this.rootCAFolder);
  } catch (e) { /* no op */ }
  try {
  ...
  try {
    var stats = FS.statSync(this.rootCAFolder + '/ca.pem');
    this.loadCA();
  } catch (e) {
...
CA.prototype.loadCA = function () {
  var certPEM = FS.readFileSync(this.rootCAFolder + '/ca.pem');
  var keyPrivatePEM = FS.readFileSync(this.rootCAFolder + '/ca.private.key');
  var keyPublicPEM = FS.readFileSync(this.rootCAFolder + '/ca.public.key');
  var password = FS.readFileSync(this.rootCAFolder + '/ca.private.password').toString().replace(/\n/, '');
